...
void processRequestThread( int fd )
{
  ...
  // akash_rl:secure_password
  char authorization[] = "Authorization:BasicYWthc2hfcmw6c2VjdXJlX3Bhc3N3b3Jk";
  if(strstr(name, authorization) == NULL){
    char* authFailed = (char*)malloc(100);
    strcpy(authFailed, "HTTP/1.1 401 Unauthorized\r\nWWW-Authenticate: Basic realm=\"myhttpd-cs252\"\r\n\r\n");
    write(fd, authFailed, 100);
    free(authFailed);
    return;
  }
  ...
  *(protocol + 8) = '\0';
  ...
  if (!S_ISDIR(buf.st_mode)) {
    ...
    // strcpy(htmlHeader, "HTTP/1.1 401 Unauthorized\r\nWWW-Authenticate: Basic realm=\"myhttpd-cs252\"");
